...
<?php include ("../includes/time_function.php"); ?>
...
<?php include "../includes/dieim.php"; ?>
<div class="w3-bottom">
...
echo "<div style='display: inline;'><a href='../".$check_path."posts/post?pid=".$get_post_id."' class=\"username_OF_postLink\">";
        if (!empty($get_post_img)) {
			if(preg_match("/.(png|jpeg|jpg)$/i", $get_post_img/*importent!!!*/)){
				if(!empty($img_op)){
        echo "<img align='center' style='opacity:$img_op;' class='eximg $img_sty' onclick='lightbox(\"$get_post_id\")' id='lightboxImg_$get_post_id' src=\"../".$imgs_path."$get_post_img\" alt='$query_fetch_fullname' />";
			}else{
		echo "<img loading='lazy' align='center' class='eximg $img_sty' onclick='lightbox(\"$get_post_id\")' id='lightboxImg_$get_post_id' src=\"../".$imgs_path."$get_post_img\" alt='$query_fetch_fullname' />";

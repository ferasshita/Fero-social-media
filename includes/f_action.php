...
$m_hide = "1";
$time = time();
	$fer_sql = "INSERT INTO messages (m_id,message,m_from,m_to,m_time,m_seen,m_hide) VALUES (:m_id,:mes,:sid,:user_id,:time,:m_seen,:m_hide)";
    $fer = $conn->prepare($fer_sql);
    $fer->bindParam(':m_id',$m_id,PDO::PARAM_INT);
    $fer->bindParam(':mes',$mes,PDO::PARAM_INT);
    $fer->bindParam(':sid',$s_id,PDO::PARAM_STR);
    $fer->bindParam(':user_id',$user_id,PDO::PARAM_STR);
    $fer->bindParam(':time',$time,PDO::PARAM_STR);
    $fer->bindParam(':m_seen',$m_seen,PDO::PARAM_STR);
    $fer->bindParam(':m_hide',$m_hide,PDO::PARAM_STR);
    $fer->execute();
		$fer_sql = "INSERT INTO messages (m_id,message,m_from,m_to,m_time,m_seen,m_hide) VALUES (:m_id,:mes,:sid,:user_id,:time,:m_seen,:m_hide)";
    $fer = $conn->prepare($fer_sql);
    $fer->bindParam(':m_id',$m_id,PDO::PARAM_INT);

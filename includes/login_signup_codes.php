...
echo "<p>".lang('enter_username_to_login')."</p>";
}elseif ($username == null){
    echo "<p>".lang('enter_username_to_login')."</p>";
}elseif($password == null){
    echo "<p>".lang('enter_password_to_login')."</p>";
}else{
    $chekPwd = $conn->prepare("SELECT * FROM signup WHERE Username = :username OR Email= :email OR phone =:phone ");
    $chekPwd->bindParam(':email', $username, PDO::PARAM_STR);
    $chekPwd->bindParam(':username',$username,PDO::PARAM_STR);
    $chekPwd->bindParam(':phone',$username,PDO::PARAM_STR);
    $chekPwd->execute();
    ...
    if (isset($_COOKIE['linAtt']) AND $_COOKIE['linAtt'] == $username) {
        echo "<p>".lang('cannot_login_attempts').".</p>";
    }else{
    ...
    if ($un_or_em != $username) {
        echo "<p>".lang('un_email_not_exist')."!</p>";
        ...
        echo "<p>".lang('password_incorrect_you_have')." $LoginTry ".lang('attempts_to_login')."</p>";
    ...
    $num = $query->rowCount();
    if($num == 0){
        echo "<p'>".lang('un_and_pwd_incorrect')."!</p>";
    }else{
        $_SESSION['attempts'] = 0;
...
$phone = filter_var(htmlentities($_POST['ph']),FILTER_SANITIZE_STRING);
// =========================== password hashinng ==================================
...
if($signup_fullname == null || $signup_username == null || $signup_password == null || $signup_cpassword == null){
       echo "<p>".lang('please_fill_required_fields')."</p>";
}elseif($num_un_ex == 1){
       echo "<p>".lang('user_already_exist')."</p>";
}elseif(strlen($signup_password) < 6){
    echo "<p>".lang('password_short').".</p>";
}elseif($signup_password_var != $signup_cpassword){
    echo "<p>".lang('password_not_match_with_cpassword')."</p>";
...
}elseif(!$phone){if (!filter_var($signup_email, FILTER_VALIDATE_EMAIL)) {
    echo "<p>".lang('invalid_email_address')."</p>";
}}else{
    ...
    $signup_admin = "1";   
    $signupsql = "INSERT INTO signup (id,Fullname,Username,Email,phone,Password,Userphoto,gender,language,admin)
    VALUES( :signup_id, :signup_fullname, :signup_username, :signup_email, :phone, :signup_password, :userphoto, :signup_gender,:signup_language,:signup_admin)";
    ...
    $signupsql = "INSERT INTO signup (id,Fullname,Username,Email,phone,Password,Userphoto,gender,language)
    VALUES( :signup_id, :signup_fullname, :signup_username, :signup_email, :phone, :signup_password, :userphoto, :signup_gender,:signup_language)";
    $query = $conn->prepare($signupsql);
    $query->bindParam(':signup_id', $signup_id, PDO::PARAM_INT);
    $query->bindParam(':signup_fullname', $signup_fullname, PDO::PARAM_STR);
    $query->bindParam(':phone', $phone, PDO::PARAM_STR);
    ...
    $loginsql = "SELECT * FROM signup WHERE (Username= :signup_username OR Email= :signup_email OR phone =:phone) AND Password= :signup_password";
    $query = $conn->prepare($loginsql);
    $query->bindParam(':signup_username', $signup_username, PDO::PARAM_STR);
    $query->bindParam(':signup_email', $signup_email, PDO::PARAM_STR);
    $query->bindParam(':phone', $phone, PDO::PARAM_STR);

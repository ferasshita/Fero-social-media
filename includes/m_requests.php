...
//users
$hashtags_url = '/(\@)([x00-\xFF]+[a-zA-Z0-9x00-\xFF_\w]+)/';
$message_body = preg_replace($hashtags_url, '<a href="../u/$2" title="$2">@$2</a>', $message_body);
//links
$hashtags_url = '/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\:[0-9]+)?(\/\S*)?/';
$message_body = preg_replace($hashtags_url, '<a href="$0" target="_blank" title="$0">$0<i class="fa fa-link"></i></a>', $message_body);
//post imgs
$hashtags_url = '/(embed)\/(post)\?(pid)\=[0-9\-\.]+[0-9a-zA-Z]{2,3}?/';
$message_body = preg_replace($hashtags_url, '<iframe src="../$0"></iframe>', $message_body);
$message_body = nl2br($message_body);
// send result
//messages place//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////
echo "
<table class='m_msgTable' id='m_$msg_id' data-count='".$getMsgsCout."' style='direction:".$userDir.";'>
	<tr>
		".$msgUserPhoto."
		<td>
			<div oncontextmenu=\"del('$msg_id')\" class='".$m_msgU."' style='direction:".lang('dir').";' title='".date("d M, Y",$msgs_row['m_time'])."'>

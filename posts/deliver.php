...
<!--=============================[ NavBar ]========================================-->
        ...
        $sid = $_SESSION['id'];
        $fPosts_sql_sql = "SELECT * FROM buy WHERE author_id = :story_id";
...
<table style='width:100%;'>
    <tr>
    <td style='width:50px;'>
    <div class='username_OF_postImg'><img src=\"../imgs/user_imgs/$po_up\"></div><td>
    <td>
    <a href='".$check_path."u/$po_un' class='username_OF_postLink'>$po_fu</a><br/>
    <a href='".$check_path."posts/post?pid=".$get_post_id."' class='username_OF_postTime'>$timeago</a>
    </td>
    <td style='float:right;'>
<a href='tel:$po_tel'><button style='background:#00CCFF;color:white;font-size:25px;border-radius:4px;border:none;'>call</button></a>
    </td>
    </tr>
    </table>
";

$qsql = "SELECT * FROM wpost WHERE post_id=:post_id";
$view_posts = $conn->prepare($qsql);
$view_posts->bindParam(':post_id', $post_ido, PDO::PARAM_INT);
$view_posts->execute();
...
while ($query_fetch = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $query_fetch_phone = $query_fetch['phone'];
}
...
//shared posts///////////////////////////////////////////////////////////////////////////////
    ...
    <a href='".$check_path."posts/post?pid=".$get_post_id."' class='username_OF_postTime'>$timeago</a>
    <a href='tel:$query_fetch_phone' class='username_OF_postTime'> &bull; call</a>
    </td>
    <td>
    <div class='dropdown'>
...
<a href=\"tel:$shU_ph\" class=\"username_OF_postTime\"> &bull; call</a>
</td>
</tr>
</tbody></table>
</div><div id='postTitle_$get_post_id'>";
...
var uluru = {lat: <?php echo"$lat"; ?>, lng: <?php echo"$longi"; ?>};
...
var marker = new google.maps.Marker({position: uluru, map: map});
    ...
    <script defer
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDzCmUQ94RwLxjQmVFBCjfGMcn-CN-FXi8&callback=initMap">
    </script>
<?php
$all_users_sql = "SELECT * FROM buy WHERE post_id=:post_id AND author_id=:author_id";
$all_users = $conn->prepare($all_users_sql);
$all_users->bindParam(':post_id',$post_ido,PDO::PARAM_STR);
$all_users->bindParam(':author_id',$story_id,PDO::PARAM_STR);
$all_users->execute();
$view_postsNum = $all_users->rowCount();
if($view_postsNum < 1){
	echo"<span id='buy_$post_ido' class='w3-bottom' style='cursor:pointer;min-width:100%'><span class=\"unfollow_btn\" style='min-width:100%' onclick=\"acce('$post_id','$story_id')\"><span class=\"fa fa-check\"></span> Accept</span></span>";
}else{
while ($fetch_users = $all_users->fetch(PDO::FETCH_ASSOC)) {
$post_idi = $fetch_users['post_id'];
$author_idi = $fetch_users['author_id'];
$cql = "SELECT * FROM buy WHERE post_id=:post_id AND author_id=:author_id";
$c = $conn->prepare($cql);
$c->bindParam(':post_id',$post_ido,PDO::PARAM_STR);
$c->bindParam(':author_id',$story_id,PDO::PARAM_STR);
$c->execute();
$c_num = $c->rowCount();
if ($c_num == 1){
    $follow_btn = "<span id='buy_$post_idi' class='w3-bottom' style='cursor:pointer;min-width:100%'><span class=\"unfollow_btn\" style='min-width:100%' onclick=\"acce('$post_ido','$story_id')\"><span class=\"fa fa-check\"></span> accept</span></span>";
}else{
    $follow_btn = "<span id='buy_$post_idi' class='w3-bottom' style='cursor:pointer;min-width:100%'><span class=\"follow_btn\" style='min-width:100%' onclick=\"acce('$post_ido','$story_id')\"><span class=\"fa fa-plus-circle\"></span> accepted</span></span>";
}
    $fbtn = $follow_btn;
echo "

...
//////////////////cor_news
if (isset($_POST['cor_news_btn'])) {

    $num_cor = htmlspecialchars(htmlentities($_POST['num_cor']));
    $loc_con = htmlspecialchars(htmlentities($_POST['loc_con']));
    $type = htmlspecialchars(htmlentities($_POST['type']));
	$in_time = time();
	if($type == "case"){
$iptdbsql = "UPDATE corona SET ty_cas=:num_cor, time=:in_time WHERE region =:loc_con";
$insert_post_toDB = $conn->prepare($iptdbsql);
$insert_post_toDB->bindParam(':num_cor', $num_cor,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':in_time', $in_time,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':loc_con', $loc_con,PDO::PARAM_STR);
$insert_post_toDB->execute();
	}elseif($type == "death"){
		$iptdbsql = "UPDATE corona SET ty_dea=:num_cor, time=:in_time WHERE region =:loc_con";
$insert_post_toDB = $conn->prepare($iptdbsql);
$insert_post_toDB->bindParam(':num_cor', $num_cor,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':in_time', $in_time,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':loc_con', $loc_con,PDO::PARAM_STR);
$insert_post_toDB->execute();
				...
				$iptdbsql = "UPDATE corona SET ty_hea=:num_cor, time=:in_time WHERE region =:loc_con";
$insert_post_toDB = $conn->prepare($iptdbsql);
$insert_post_toDB->bindParam(':num_cor', $num_cor,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':in_time', $in_time,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':loc_con', $loc_con,PDO::PARAM_STR);
...
//////////////////update news
if (isset($_POST['newsab_btn'])) {

    $newsab = htmlspecialchars(htmlentities($_POST['newsab_fi']));
$iptdbsql = "UPDATE news_ab SET newsab_fi=:newsab";
$insert_post_toDB = $conn->prepare($iptdbsql);
$insert_post_toDB->bindParam(':newsab', $newsab,PDO::PARAM_STR);
$insert_post_toDB->execute();
}
...
//////////////////posts
if (isset($_POST['posts_ne'])) {
	$post_id = rand(0,9999999)+time();
    $p_content = htmlspecialchars(htmlentities($_POST['p_content']));
    $link = htmlspecialchars(htmlentities($_POST['link']));
    $linktext = htmlspecialchars(htmlentities($_POST['linktext']));
    $imagefo = htmlspecialchars(htmlentities($_POST['imagefo']));
	$p_time = time();
	if($imagefo == "update"){
	$uimage = "image1.jpg";
	}
	elseif($imagefo == "pandimic"){
		$uimage = "image2.jpg";
	}
	if($link){
		$iptdbsql = "INSERT INTO postcom
(post_id,p_content,link,linktext,time,uimage)
VALUES
( :post_id, :p_content, :link, :linktext, :p_time, :uimage)
";
$insert_post_toDB = $conn->prepare($iptdbsql);
...
$insert_post_toDB->bindParam(':linktext', $linktext,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':uimage', $uimage,PDO::PARAM_STR);
$insert_post_toDB->execute();
	}else{
$iptdbsql = "INSERT INTO postcom
(post_id,p_content,time,uimage)
VALUES
( :post_id, :p_content, :p_time, :uimage)
";
$insert_post_toDB = $conn->prepare($iptdbsql);
$insert_post_toDB->bindParam(':post_id', $post_id,PDO::PARAM_STR);
$insert_post_toDB->bindParam(':p_time', $p_time,PDO::PARAM_INT);
...
<head>
    <title><? echo lang('dashboard'); ?> &bull; Fero</title>
    <meta charset="UTF-8">
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
            ...
            <li>
                <a href="index?adb=Support_box">
                    <p class="<? if($urlP=='Support_box'){echo'dboard_lActive';} ?>"><? echo lang('supportBox'); ?></p>
			            ...
			            <li>
                <a href="index?adb=corona">
                    <p class="<? if($urlP=='corona'){echo'dboard_lActive';} ?>">corona</p>
                </a>
            </li>
				<li>
                <a href="index?adb=newsab">
                    <p class="<? if($urlP=='newsab'){echo'dboard_lActive';} ?>">Our next update</p>
                </a>
            </li>
				<li>
                <a href="index?adb=post">
                    <p class="<? if($urlP=='post'){echo'dboard_lActive';} ?>">posts for the community</p>
                </a>
            </li>
        </ul>
    </div>
    ...
    <?php ; break; ?>
	    <?php case 'corona': ?>
<p class="dashboard_path"><a href="?adb=General"><? echo lang('dashboard'); ?></a> / corona virus news</p>
    <form action="" method="post">
        <p><input type="number" name="num_cor" placeholder="Number" class="dashboardField" required></p>
        <br>
        <p>
<select class="dashboardField" name="loc_con">
<option value="tripoli">tripoli</option>
<option value="banigazi">banigazi</option>
<option value="misrata">misrata</option>
<option value="sabha">sabha</option>
<option value="sirt">sirt</option>
<option value="alkofra">alkofra</option>
<option value="zletin">zletin</option>
<option value="alzawia">alzawia</option>
<option value="albada">albada</option>
<option value="zwara">zwara</option>
<option value="tobroq">tobroq</option>
<option value="giryan">giryan</option>
</select>

        </p><br>
		        <p>
<select class="dashboardField" name="type">
<option value="case">case</option>
<option value="death">death</option>
<option value="health">health</option>
</select>
        </p><br>
        <p><input type="submit" name="cor_news_btn" class="btn_blue" value="<? echo lang('submit'); ?>" /></p>
        
    </form>
    <?php ; break; ?>
		    <?php case 'newsab': ?>
<p class="dashboard_path"><a href="?adb=General"><? echo lang('dashboard'); ?></a> / Our next update</p>
    <form action="" method="post">
        <p><textarea name="newsab_fi" placeholder="Our next update" class="dashboardField" required><?php echo"$newsab_fi_di"; ?></textarea></p>
        <br>
        <p><input type="submit" name="newsab_btn" class="btn_blue" value="<? echo lang('submit'); ?>" /></p>
    </form>
    <?php ; break; ?>
			    <?php case 'post': ?>
<p class="dashboard_path"><a href="?adb=General"><? echo lang('dashboard'); ?></a> / post</p>
    <form action="" method="post">
        <p><textarea name="p_content" placeholder="post for the community" class="dashboardField" required></textarea></p>
        <p><input type="text" name="link" placeholder="link" class="dashboardField"></p>
        <p><input type="text" name="linktext" placeholder="link text" class="dashboardField"></p>
        <p><select type="text" name="imagefo" placeholder="link text" class="dashboardField">
		<option value="update">update</option>
		<option value="pandimic">pandimic</option>
		</select></p>
        <br>
        <p><input type="submit" name="posts_ne" class="btn_blue" value="<? echo lang('submit'); ?>" /></p>
    </form>
    <?php ; break; ?>
    <!--///////////////////////////////////////////////-->
